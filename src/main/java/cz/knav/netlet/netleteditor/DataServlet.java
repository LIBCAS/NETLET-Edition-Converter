    ...
    INDEX_HIKO {
      ...
      JSONObject doPerform(HttpServletRequest req, HttpServletResponse response) throws Exception {
        ...
        try {
          ...
          json.put("identities", hi.indexIdentities(null));
          //json.put("global_places", hi.indexGlobalPlaces());
    ...
    INDEX_TENANT_PLACES {
      ...
      JSONObject doPerform(HttpServletRequest req, HttpServletResponse response) throws Exception {
        ...
        try (SolrClient client = new HttpJdkSolrClient.Builder(Options.getInstance().getString("solr")).build()) {
          HikoIndexer hi = new HikoIndexer();
          hi.indexTenantPlaces(client, json, req.getParameter("tenant"));
...
//        try {
//          HikoIndexer hi = new HikoIndexer();
//          json.put("places", hi.indexTenantPlaces());
//        } catch (Exception ex) {
//          LOGGER.log(Level.SEVERE, null, ex);
//          json.put("error", ex.toString());
//        }
    ...
    INDEX_PROFESSIONS {
      ...
      JSONObject doPerform(HttpServletRequest req, HttpServletResponse response) throws Exception {
        ...
        try {
          HikoIndexer hi = new HikoIndexer();
          json.put("global-Professions", hi.indexGlobalProfessions());
          json.put("Professions", hi.indexProfessions());
        } catch (IOException | InterruptedException | URISyntaxException | JSONException ex) {
          LOGGER.log(Level.SEVERE, null, ex);
          json.put("error", ex.toString());
